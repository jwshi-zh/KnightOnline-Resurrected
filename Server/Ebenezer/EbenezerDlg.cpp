...
BOOL CEbenezerDlg::AISocketConnect(int zone, int flag)
{
	...
	if(flag == 1)	SetByte(pBuf, 1, send_index);
	else			SetByte(pBuf, 0, send_index);
...
BOOL CEbenezerDlg::InitializeMMF()
{
	...
	m_hMMFile = CreateFileMapping((HANDLE)-1, &security, PAGE_READWRITE | SEC_NOCACHE | SEC_COMMIT, 0, filesize, "KNIGHT_DB");
	...
	if (m_hMMFile != NULL && GetLastError() == ERROR_ALREADY_EXISTS)
	{
		...
		if (m_hMMFile == NULL)
		{
			m_StatusList.AddString("Shared Memory Load Fail!!");
	...
	m_StatusList.AddString("Shared Memory Create Success!!");
...
void CEbenezerDlg::GetTimeFromIni()
{
	...
	inipath.Format("%sServer.ini", GetProgPath());
...
void CEbenezerDlg::SendAllUserInfo()
{
	...
	for(int i=0; i<m_PartyArray.GetSize(); i++)	{
		...
		for( int j=0; j<8; j++ ) {
			...
			//SetShort(send_buff, pParty->sHp[j], send_index );
			//SetByte(send_buff, pParty->bLevel[j], send_index );
			//SetShort(send_buff, pParty->sClass[j], send_index );
		}
...
/*	if( m_byBattleOpen == NO_BATTLE )	{	// When Battlezone is closed, open it!
		if( nWeek == m_nBattleZoneOpenWeek && nTime == m_nBattleZoneOpenHourStart )	{
			BattleZoneOpen(BATTLEZONE_OPEN);
//			KickOutZoneUsers(ZONE_FRONTIER);	// Kick out users in frontier zone.
		}
	}
	else {	  // When Battlezone is open, close it!
		if( nWeek == (m_nBattleZoneOpenWeek+1) && nTime == m_nBattleZoneOpenHourEnd )	{
			m_byBanishFlag = 1;
		}
	}	*/
...
void CEbenezerDlg::BattleZoneCurrentUsers()
{
	...
	if( !pMap )		return;
	if( m_nServerNo != pMap->m_nServerNo )	return;
