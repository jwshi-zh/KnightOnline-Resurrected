...
BOOL CAujardDlg::OnInitDialog()
{
	...
	m_nServerNo = GetPrivateProfileInt("ZONE_INFO", "GROUP_INFO", 2, inipath);
	m_nZoneNo = GetPrivateProfileInt("ZONE_INFO", "ZONE_INFO", 2, inipath);
...
BOOL CAujardDlg::InitializeMMF()
{
	...
	m_lpMMFile = (char*)MapViewOfFile(m_hMMFile, FILE_MAP_WRITE, 0, 0, filesize);
	if (!m_lpMMFile)
		return FALSE;
