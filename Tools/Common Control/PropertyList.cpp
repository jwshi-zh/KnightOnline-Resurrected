...
void CPropertyList::DrawItem(LPDRAWITEMSTRUCT lpDIS) 
{
	...
	if (nIndex != (UINT) -1)
	{
		...
		{
			dc.FillSolidRect(CRect(rect.left+2,rect.top+2,rect.right-4,rect.bottom-4), pItem->m_crColor); // Drawing background color..
		}
...
void CPropertyList::OnSelchangeCmbBox()
{
	...
	if (m_ComboBox)
	{
		...
		CWnd* pWnd = GetParent();
		if(pWnd) pWnd->SendMessage(WM_NOTIFY, (DWORD)this, (DWORD)pItem); // Send message to parent window.
	}
}
	...
	else if (pItem->m_nItemType == PIT_FILE_MULTI)
	{
		...
		if(IDOK == dlg.DoModal())
		{
			...
			if(iSC > 0)
			{
				...
				for(int i = 1; i < iSC-1; i++) // The reason for starting from 1 is that the beginning and ending file names change when multiple files are called in the file dialog box.
				{
