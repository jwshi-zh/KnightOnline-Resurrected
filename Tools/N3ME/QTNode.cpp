...
// connect before friends are made
// Because there is an attempt, all nodes
// After creating everything, try connecting as a friend.
//
...
{
	if (m_Level == m_pRefTerrain->m_iMaxLevel) // When it's the last node...
	{
		for (int x = -1; x < 2; x++)
		{
			for (int z = -1; z < 2; z++)
...
//
// tick...
// The substance of the quad tree...^^
// First, check if it is within the visible area (IntersectRect()..) and...
// If it is within the visible area, check whether to draw or go further..
//
void CQTNode::Tick()
{
	...
	RECT rt;
	int Range = 1 << (m_pRefTerrain->m_iMaxLevel - m_Level); // Distance from the center point to the edge.
	...
	if (IntersectRect(&rt1, &rt, &(m_pRefTerrain->m_VisibleRect)) == 0)
		return; // If not in the visible area, exit.

	// Check whether to draw or go further..
...
// return true -> stop going down Render;
// return false -> further down...
// This is the core of the terrain detail method...^^
//
...
//
// Simple distance calculation method..
//
...
//
// Function that draws the most detailed tile..
//
	...
	for (i = 0; i < 2; i++)
	{
		...
		if (m_pRefTerrain->m_ppIsLightMap[cx][cz])
		{
			...
			if (!pLightMapVertices)
				m_pRefTerrain->m_LightMapVB->Lock(0, 0, (BYTE **)&pLightMapVertices, 0);
		...
		if ((m_pRefTerrain->m_ppMapData[cx][cz].DTexInfo1.Attr.Group <= 0) || m_pRefTerrain->GetTileTex(tmpTIdx) == NULL) // When there are no tiles...
		{
		...
		else
		{
			...
			else // In case of purely tile taking..
			{
				dir1 = m_pRefTerrain->m_ppMapData[cx][cz].DTexInfo1.Dir;
