...
void CWallMgr::AddWall2Coll(CN3ShapeMgr* pShapeMgr)
{
	...
	for(itWall = m_pWalls.begin(); itWall != m_pWalls.end(); itWall++)
	{
		...
		if(!pWall)
		{
			fprintf(stream, "Ah, there's no wall, so I'm trying...ㅡ.ㅡ\n");
			continue;
		}
		...
		for(itVertex = pWall->m_Wall.begin(); itVertex != pWall->m_Wall.end(); itVertex++)
		{
			...
			if(!pShapeMgr->AddCollisionTriangle(v1, v2, v3)) fprintf(stream, "I couldn't put the wall..ㅡ.ㅡ\n");
			if(!pShapeMgr->AddCollisionTriangle(v1, v3, v2)) fprintf(stream, "I couldn't put the wall..ㅡ.ㅡ\n");
