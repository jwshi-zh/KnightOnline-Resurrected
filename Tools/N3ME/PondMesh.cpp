...
void CPondMesh::Release()
{
	...
	m_iWaterScaleWidth = 4;	 // number of points in the horizontal direction
	m_iWaterScaleHeight = 6; // Number of vertical points
	m_iBackUpWidht = 0;
	m_iBackUpHeight = 0;
...
void CPondMesh::Render()
{
	...
	if (m_pTexture && m_iVC > 2 && m_iIC > 0)
	{
		...
		if ((m_dwPondAlpha & 0xff000000) != 0xff000000) // set the alpha factor
		{
			// set render state
			...
			s_lpD3DDev->SetRenderState(D3DRS_TEXTUREFACTOR, m_dwPondAlpha); // set the alpha factor
		...
		// render
		s_lpD3DDev->DrawIndexedPrimitiveUP(D3DPT_TRIANGLELIST, 0, m_iVC, m_iIC, m_pdwIndex, D3DFMT_INDEX16, m_pViewVts // m_pVertices
										   ,
...
void CPondMesh::RenderVertexPoint() // Redraw only dots to make them visible
{
	...
	for (i = 0; i <= m_iVC; ++i)
	{
		...
		{
			// set X (If you mark a dot, it only takes 1 pixel, so draw an X mark.
...
void CPondMesh::MakeDrawRect(__Vector3 *p4vPos) // Create the border of the area of the pond that will be visible on the screen
{
	if (p4vPos == NULL)
		return;

	memcpy(m_vDrawBox, p4vPos, sizeof(__Vector3) * 4);
	m_fWaterHeight = p4vPos[0].y; // get the height of the water
	p4vPos = NULL;				  // I'm afraid I'll get weird results from memory...

	//------------------------------------------------ ----------------------------
	DWORD color = 0xffffff00;
	m_ViewRect[0].Set(m_vDrawBox[0], color); // locus
	m_ViewRect[1].Set(m_vDrawBox[1], color); // locus
...
float CPondMesh::SettingDrawPos(__Vector3 vPos1, __Vector3 vPos2, __Vector3 vPos3, __Vector3 vPos4, int iLinePosNum)
{
	...
	fLength /= (iLinePosNum + 1); // Find the length, calculate the points to be located excluding the point to be inserted at the end
	LinePos.Normalize();
	LinePos *= fLength; // Get a vector with a length of 1m and enter the distance between the points
...
void CPondMesh::MakePondPos()
{
	...
	ASSERT(MAX_PONDMESH_VERTEX > m_iWaterScaleWidth * m_iWaterScaleHeight); // Check if the number of points to be created reaches max
	int ix, iz, item;

	// after receiving the location of the points in the direction
	float *fX, fY, *fZ;
...
void CPondMesh::MakeIndex()
{
	...
	int x = 0, y = m;
	WORD *indexPtr = m_pdwIndex; // set the position to call the triangle
...
{
	__Vector3 pBakVertices[MAX_PONDMESH_VERTEX];	// Value used for storage and backup
	__VertexXyzT2 pBakViewVts[MAX_PONDMESH_VERTEX]; // value to be displayed on the screen
...
void CPondMesh::InputSelectPos(float fX, float fY, float fZ, int iVC)
{
	...
	// reset height if height is wrong
	...
	if (iVC > -1) // When selecting some parts, not all
	{
		...
		// Temporarily hold selected points
...
BOOL CPondMesh::InputDummyMovingPos(__Vector3 vDummyMovingPos, BOOL bMovePond)
{
	...
	// Movement of the selected points area
	...
	else
	{
		...
		if (vDummyMovingPos.x != 0 || vDummyMovingPos.z != 0)
			MovingPos(); // Move the dots according to the pile's movement
	}
...
// By default, the selected points are already moved, and the expected points to move are calculated by referring to m_pVertices, which can be called the original.
...
void CPondMesh::CalcuWidth(int iSx, int iSy, int iEx, int iEy)
{
	...
	//------------------------------------------------ ----------------------------------
	// get the first point to compute
	__Vector3 vBakPick = *(m_pVertices + iHeightNum + iSx); // Coordinates before being backed up (calculate new coordinates by referring to them)
	__Vector3 vNowPick = *(m_pViewVts + iHeightNum + iSx);	// currently assigned point (to be assigned multiple later)
	__Vector3 *pLRVertices = m_pVertices + iHeightNum;		// first point to compute (for backup)
	__VertexXyzT2 *pLRViewVer = m_pViewVts + iHeightNum;	// first point to compute (for now)
	//------------------------------------------------ ----------------------------------

	//------------------------------------------------ ----------------------------------
	// have points at both ends corresponding to positions
	__Vector3 *pvLeft, *pvRight, *pvTop, *pvBottom;
	pvRight = pLRVertices; // first start is from the right
	...
	//------------------------------------------------ ----------------------------------
	iIntervalNum = iSx - 1; // To subtract one point each from the left and right
	if (iIntervalNum > 0)
	{
		...
		{
			// Find the new position by calculating the distance ratio for each point in between
			...
			if (fx2 != 0)
			{
				...
				// right end point x-backup point x : right end point x-current calculated point x = right end point x-new point x : find point x
				fnx2 = pvRight->x - vNowPick.x;
				fnx1 = fnx2 * ftemp;
				pLRViewVer->x = pvRight->x - fnx1; // input the new point of x

				// right end point x-backup point x : right end point x-current calculated point x = new point z-backup point z : amount of change to be obtained z
				fny1 = fny2 * ftemp;
				pLRViewVer->z += fny1; // add the change in z
			}
		}
	}
	//------------------------------------------------ ----------------------------------
	...
	//------------------------------------------------ ----------------------------------
	iIntervalNum = m_iWaterScaleWidth - iSx - 2; // To subtract one point each from the left and right
	if (iIntervalNum > 0)
	{
		...
		for (j = 0; j < iIntervalNum; ++j, ++pLRVertices, ++pLRViewVer, ++pvTop, ++pvBottom)
		{
			...
			if (fx2 != 0)
			{
				...
				fnx1 = fnx2 * ftemp;
				pLRViewVer->x = pvLeft->x + fnx1; // input the new point of x
...
void CPondMesh::SetAllPos(int iSx, int iSy, int iEx, int iEy)
{
	...
	vCenter = *(m_pVertices + m_iWaterScaleWidth * iSy + iSx);	 // currently assigned point (to be assigned multiple later)
	vNowCenter = *(m_pViewVts + m_iWaterScaleWidth * iSy + iSx); // Coordinates before being backed up (calculate new coordinates by referring to them)
	...
	if (iIntervalNum > 0)
	{
		...
		if (fy2 != 0)
		{
			...
			for (j = 0; j < iIntervalNum; ++j, pTBVertices += m_iWaterScaleWidth, pTBViewVer += m_iWaterScaleWidth)
			{
				...
				fnx1 = fnx2 * ftemp;
				pTBViewVer->x += fnx1; // add the change in x
...
bool CPondMesh::Load1001(HANDLE hFile)
{
	...
	ReadFile(hFile, m_vDrawBox, sizeof(m_vDrawBox), &dwNum, NULL); // number of dots in a line
	...
	MakeDrawRect(m_vDrawBox); // set area line
...
bool CPondMesh::Load1000(HANDLE hFile)
{
	...
	ReadFile(hFile, &m_iWaterScaleWidth, sizeof(m_iWaterScaleWidth), &dwNum, NULL); // number of dots in a line
...
bool CPondMesh::Load(HANDLE hFile)
{
	...
	ReadFile(hFile, &m_iWaterScaleWidth, sizeof(int), &dwNum, NULL); // number of dots in a line
...
bool CPondMesh::Save(HANDLE hFile)
{
	...
	WriteFile(hFile, m_vDrawBox, sizeof(m_vDrawBox), &dwNum, NULL); // number of dots in a line
...
void CPondMesh::SetVtx()
{
	...
	// readjust (to get it right on screen)
	ReCalcUV();
...
int CPondMesh::AddVertex(__Vector3* pvPos)
{
	...
	// ASSERT(m_iVC<2);
	ASSERT(MAX_PONDMESH_VERTEX>m_iVC+m_iWaterScaleWidth*m_iWaterScaleHeight); // Check if the number of points to be created reaches max

	// Cut 4m each between the first starting point and the second point
...
int CPondMesh::AddVertex()
{
	...
	// Finding the direction orthogonal to the last two existing points (taken at a distance of 4m)
