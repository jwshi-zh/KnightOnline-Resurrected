...
struct __DUMMYCUBE
{
	int iType;		// Center , X, Y, Z type distinction.
...
public:
protected:
	__DUMMYCUBE			m_DummyCubes[NUM_DUMMY];	// 4cubes(center, x, y, z);
	__VertexXyzColor	m_LineVertices[6];			// The points that make up the line connecting the four cubes
	CTypedPtrArray<CPtrArray, CN3Transform*> m_SelObjArray;	// MapMngObjects selected from
