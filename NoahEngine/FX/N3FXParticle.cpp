...
bool CN3FXParticle::Tick()
{
	...
	else
	{
		...
		{
			auto Alpha = (DWORD)(255.0f * m_fCurrLife / m_pRefParent->m_fFadeIn);
			Alpha = (Alpha<<24);
		...
		{
			auto Alpha = (DWORD)(255.0f * ((m_pRefParent->m_fFadeIn + m_fLife + m_pRefParent->m_fFadeOut)-m_fCurrLife) / m_pRefParent->m_fFadeOut);
			Alpha = (Alpha<<24);
			m_dwColor = Alpha + 0x00ffffff;
