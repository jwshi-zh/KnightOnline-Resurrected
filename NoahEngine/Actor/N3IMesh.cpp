...
CN3IMesh::~CN3IMesh()
{
	...
	delete [] m_pwVtxIndices; m_pwVtxIndices = nullptr;
	delete [] m_pwUVsIndices; m_pwUVsIndices = nullptr;
...
bool CN3IMesh::Create(int nFC, int nVC, int nUVC)
{
	...
	if(nUVC > 0)
	{
		m_nUVC = nUVC; m_pfUVs = new float[nUVC*2]; memset(m_pfUVs, 0, 8 * nUVC); // The reason why the size is 8 is because there are 2 floats.
