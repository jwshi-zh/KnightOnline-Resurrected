...
CN3TableBase<__TABLE_ZONE>*				CGameBase::s_pTbl_Zones = nullptr;				// Zone 정보에 관한 Table
CN3TableBase<__TABLE_UI_RESRC>*			CGameBase::s_pTbl_UI = nullptr;				// UI FileName Table
CN3TableBase<__TABLE_ITEM_BASIC>*		CGameBase::s_pTbl_Items_Basic = nullptr;			// 각 유저의(내 자신과 주위 다른 사람) 클레스별 장착 아이템 리소스 테이블
CN3TableBase<__TABLE_ITEM_EXT>*			CGameBase::s_pTbl_Items_Exts[MAX_ITEM_EXTENSION] = 
											{nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr};			// 각 유저의(내 자신과 주위 다른 사람) 클레스별 장착 아이템 리소스 테이블
CN3TableBase<__TABLE_PLAYER_LOOKS>*		CGameBase::s_pTbl_UPC_Looks = nullptr;			// 각 유저의(내 자신과 주위 다른 사람) 클레스별 기본 스킨
CN3TableBase<__TABLE_PLAYER_LOOKS>*		CGameBase::s_pTbl_NPC_Looks = nullptr;			// NPC, Mob 기본 모습 - 6개의 캐릭터 파트(), 2개의 플러그
CN3TableBase<__TABLE_UPC_SKILL>*		CGameBase::s_pTbl_Skill = nullptr;				// Skill 정보에 관한 Table
CN3TableBase<__TABLE_EXCHANGE_QUEST>*	CGameBase::s_pTbl_Exchange_Quest = nullptr;	// 교환 퀘스트에 관한 테이블..
CN3TableBase<__TABLE_FX>*				CGameBase::s_pTbl_FXSource;		// FX소스 정보 테이블..
CN3TableBase<__TABLE_QUEST_MENU>*		CGameBase::s_pTbl_QuestMenu		= nullptr;		// 퀘스트 선택 메뉴
CN3TableBase<__TABLE_QUEST_TALK>*		CGameBase::s_pTbl_QuestTalk		= nullptr;		// 퀘스트 지문


CN3WorldManager*	CGameBase::s_pWorldMgr = nullptr;		// 월드 매니져..
CPlayerOtherMgr*	CGameBase::s_pOPMgr = nullptr;				// Other Player Manager - 다른 유저 관리 클래스..
CPlayerMySelf*		CGameBase::s_pPlayer = nullptr;			// 유저 클래스..
...
{
	// Tables ....
	delete s_pTbl_Zones; s_pTbl_Zones = nullptr;		// Zone 정보에 관한 Table
	delete s_pTbl_UI; s_pTbl_UI = nullptr;					// UI Resource File Table loading
	delete s_pTbl_UPC_Looks; s_pTbl_UPC_Looks = nullptr;	// 플레이어들의 기본 모습이 되는 NPC Resource Table loading
	...
	delete s_pTbl_Skill; s_pTbl_Skill = nullptr;					// Skill 정보에 관한 Table
	delete s_pTbl_Exchange_Quest; s_pTbl_Exchange_Quest = nullptr;	// Skill 정보에 관한 Table
	delete s_pTbl_FXSource; s_pTbl_FXSource = nullptr;				// FX Source에 관한 테이블..	
	delete s_pTbl_QuestMenu; s_pTbl_QuestMenu = nullptr;			// 퀘스트 관련 선택메뉴
	delete s_pTbl_QuestTalk; s_pTbl_QuestTalk = nullptr;		// 퀘스트 관련 지문
