...
public:
	static CN3TableBase<struct __TABLE_ZONE>*			s_pTbl_Zones;			// Table about Zone information..
	static CN3TableBase<struct __TABLE_UI_RESRC>*		s_pTbl_UI;				// UI FileName Table
	static CN3TableBase<struct __TABLE_ITEM_BASIC>*		s_pTbl_Items_Basic;		// Equipped item resource table for each class of each user (myself and others around me)
	static CN3TableBase<struct __TABLE_ITEM_EXT>*		s_pTbl_Items_Exts[MAX_ITEM_EXTENSION];		// Equipped item resource table for each class of each user (myself and others around me)
	static CN3TableBase<struct __TABLE_PLAYER_LOOKS>*	s_pTbl_UPC_Looks;		// Default skin for each class of each user (myself and others around me)
	static CN3TableBase<struct __TABLE_PLAYER_LOOKS>*	s_pTbl_NPC_Looks;		// Basic appearance of NPC, Mob - 6 character parts(), 2 plugs
	static CN3TableBase<struct __TABLE_UPC_SKILL>*		s_pTbl_Skill;			// Table for skill information
	static CN3TableBase<struct __TABLE_EXCHANGE_QUEST>*	s_pTbl_Exchange_Quest;	// A table about exchange quests..
	static CN3TableBase<struct __TABLE_FX>*				s_pTbl_FXSource;		// FX source information table..
	static CN3TableBase<struct __TABLE_QUEST_MENU>*		s_pTbl_QuestMenu;		// Quest selection menu
